    ...
    public function action_login()
    {
        ...
        if (Input::method() == 'POST') {
            ...
            if ($val->run()) {
                ...
                if (\Auth\Auth::instance()->login($email, $password)) {
        ...
        /*
          $val = $this->validate(array('username', 'password'));
          if ($val['validated'] === TRUE) {
          $email = trim($val['success']['email']);
          $password = trim($val['success']['password']);
        ...
        if (Input::method() == 'POST') {
            ...
            if ($val->run()) {
                ...
                $user = [
                    ...
                    'gender' => in_array($post['gender'], [1, 2]) ? $post['gender'] : 3,
                    'auth_code' => $this->genCode(trim($post['email']))
                ];

                $user['base_url'] = $data['base_url'];
